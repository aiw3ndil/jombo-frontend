  ...
  return (
    <div className="fixed inset-0 bg-green-900/40 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
      <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full p-8 md:p-12 border-2 border-green-50 relative overflow-hidden">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 text-[var(--brand-neutral)] hover:text-[var(--brand-blue)] transition-colors text-3xl font-light"
        >
          ×
        </button>

        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-[#f0f4f8] rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-[#e2e8f0]">
        ...
        <div className="space-y-4 mb-8">
          <div className="bg-[#f0f4f8] p-4 rounded-2xl border border-[#e2e8f0] text-center">
            <p className="text-xs font-bold text-[var(--brand-neutral)] uppercase tracking-widest mb-1">
              PROTAGONISTA
            </p>
            <p className="font-bold text-[var(--brand-dark)] text-xl">{driverName}</p>
          </div>
          ...
          <div className="bg-[#f0f4f8] p-4 rounded-2xl border border-[#e2e8f0] text-center">
            ...
            <p className="font-medium text-[var(--brand-dark)] italic">{tripRoute}</p>
          </div>
        </div>

        <form onSubmit={handleSubmit}>
          <div className="mb-8">
            <label className="block text-sm font-bold text-[var(--brand-neutral)] mb-2 text-center uppercase tracking-widest">
              {t("page.myBookings.reviewModal.ratingLabel") || "¿Cómo fue tu experiencia?"}
            </label>
            {renderStars()}
            {rating > 0 && (
              <p className="text-center text-lg font-bold text-[var(--brand-blue)] italic">
          ...
          <div className="mb-8">
            <label className="block text-sm font-bold text-[var(--brand-neutral)] mb-3 uppercase tracking-widest">
            ...
            <textarea
              ...
              rows={4}
              className="w-full border-2 border-[#e2e8f0] rounded-2xl px-5 py-4 text-[var(--brand-dark)] focus:outline-none focus:border-[#e2e8f0] font-medium placeholder:text-[var(--brand-neutral)] transition-all resize-none"
              placeholder={t("page.myBookings.reviewModal.commentPlaceholder") || "Cuéntanos detalles del viaje..."}
              maxLength={500}
            />
            <div className="flex justify-end mt-2">
              <span className="text-xs font-bold text-[var(--brand-neutral)]">
                {comment.length}/500
              </span>
            </div>
          </div>
