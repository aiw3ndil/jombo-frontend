  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-4">
        <div className="spinner"></div>
        <p className="text-[var(--brand-neutral)] font-semibold text-lg">{t("page.search.searching") || "Buscando viajes"}...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-6 px-4">
        ...
        <button
          ...
          className="bg-[var(--brand-blue)] hover:bg-[var(--brand-blue)] text-white px-8 py-3 rounded-xl font-bold text-base transition-colors"
        >
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-[#f0f4f8] border-b-2 border-[#e2e8f0] py-8 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-6">
            <h1 className="text-2xl md:text-3xl font-bold text-[var(--brand-dark)] mb-2">
              {from} <span className="text-[var(--brand-neutral)] mx-2">→</span> {to}
            </h1>
            <p className="text-sm md:text-base text-[var(--brand-neutral)] font-medium">
              {t("page.search.results") || "Explora las mejores opciones para tu trayecto"}
            </p>
          </div>

          <form onSubmit={handleSearch} className="bg-white p-4 rounded-[2.5rem] shadow-xl border-2 border-[#e2e8f0] flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative group">
              ...
              <LocationInput
                ...
                className="form-input pl-12 border-transparent bg-[#f0f4f8]/30 focus:bg-white"
              />
            </div>
            <div className="flex-1 relative group">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--brand-neutral)] group-focus-within:text-[var(--brand-blue)] transition-colors z-10">
      ...
      <section className="py-20 px-4 min-h-[60vh]">
        <div className="max-w-5xl mx-auto">
          ...
          {source === "digitransit" && (
            <div className="mb-12 p-8 bg-[#f0f4f8] border-2 border-[#e2e8f0] rounded-[2.5rem] flex items-center gap-6 animate-in fade-in slide-in-from-top-4">
              <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border border-[#e2e8f0]">
                <svg className="w-7 h-7 text-[var(--brand-blue)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div>
                <p className="text-[var(--brand-dark)] font-black text-lg">
                  {lang === "fi" ? "Ei kimppakyytejä löytynyt" : lang === "es" ? "No se encontraron viajes compartidos" : "No carpools found"}
                </p>
                <p className="text-[var(--brand-neutral)] text-lg font-medium opacity-80">
          ...
          {trips.length === 0 && externalOptions.length === 0 ? (
            <div className="text-center py-24 bg-[#f0f4f8]/30 rounded-[3rem] border-2 border-dashed border-[#e2e8f0]">
              <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-8 shadow-sm border-2 border-[#e2e8f0] text-[var(--brand-neutral)]">
                <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 9.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <h2 className="text-3xl font-bold text-[var(--brand-dark)] mb-4">{t("page.search.noResults") || "No hay viajes disponibles"}</h2>
              <p className="text-[var(--brand-neutral)] text-lg mb-8 max-w-md mx-auto font-medium">Intenta cambiar las ciudades o busca de nuevo más tarde.</p>
            ...
            <div className="space-y-10">
              ...
              {trips.map((trip) => {
                ...
                return (
                  <div key={trip.id} className="result-card group">
                    <div className="absolute top-0 left-0 w-2 h-full bg-green-500 group-hover:bg-[var(--brand-blue)] transition-colors"></div>
                    
                    <div className="flex flex-col xl:flex-row gap-10">
                      {/* Lado Izquierdo: Conductor */}
                      <div className="flex-shrink-0 flex flex-col items-center">
                        <div className="relative p-1 bg-[#f0f4f8] rounded-full border-2 border-[#e2e8f0] group-hover:border-[#e2e8f0] transition-colors">
                          <div className="w-24 h-24 rounded-full overflow-hidden shadow-inner bg-[#e2e8f0] flex items-center justify-center">
                            {driverPictureUrl ? (
                              <img
                              ...
                              <span className="text-3xl font-black text-[var(--brand-neutral)]">
                                {trip.driver.name.charAt(0).toUpperCase()}
                              </span>
                            )}
                          </div>
                          <div className="absolute -bottom-1 -right-1 w-9 h-9 bg-white rounded-full border-2 border-[#e2e8f0] flex items-center justify-center shadow-lg">
                            <svg className="w-5 h-5 text-yellow-500 fill-current" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                          </div>
                        </div>
                        <p className="mt-4 font-black text-[var(--brand-dark)] text-lg">{trip.driver.name}</p>
                        <button
                          onClick={() => handleOpenReviews(trip.driver.id, trip.driver.name)}
                          className="text-[var(--brand-blue)] hover:text-[var(--brand-dark)] text-sm font-bold uppercase tracking-widest mt-2 transition-colors cursor-pointer"
                        >
                      ...
                      <div className="flex-1 space-y-8">
                        <div className="flex flex-wrap items-center gap-4">
                          <div className="flex items-center gap-2 text-[var(--brand-blue)] font-bold bg-[#f0f4f8] px-5 py-2 rounded-full border border-[#e2e8f0] shadow-sm">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span className="text-[var(--brand-neutral)] mx-1">|</span>
                        ...
                        <div>
                          <h3 className="text-4xl font-black text-[var(--brand-dark)] leading-tight tracking-tight">
                            {trip.departure_location} <span className="text-[var(--brand-neutral)] mx-2">→</span> {trip.arrival_location}
                          </h3>
                        </div>

                        <div className="grid grid-cols-2 gap-6">
                           <div className="bg-[#f0f4f8]/50 border border-[#e2e8f0] rounded-2xl p-4">
                            <p className="text-xs font-bold text-[var(--brand-neutral)] uppercase tracking-widest mb-1">{t("page.search.availableSeats") || "Plazas Libres"}</p>
                            <p className={`text-xl font-black ${trip.available_seats === 0 ? "text-red-500" : "text-[var(--brand-dark)]"}`}>
                              {trip.available_seats} <span className="text-sm font-bold text-[var(--brand-neutral)]/60 lowercase">disponibles</span>
                            </p>
                          </div>
                          <div className="bg-[#f0f4f8]/50 border border-[#e2e8f0] rounded-2xl p-4">
                            <p className="text-xs font-bold text-[var(--brand-neutral)] uppercase tracking-widest mb-1">{t("page.search.price") || "Precio"}</p>
                            <p className="text-2xl font-black text-[var(--brand-neutral)] italic">€{Number(trip.price).toFixed(2)}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col justify-center items-stretch min-w-[240px] gap-4">
                        <button
                          ...
                          className={`w-full py-6 rounded-[1.5rem] font-bold text-lg shadow-xl transform transition-all active:scale-95 ${
                            bookingStatus === "confirmed"
                              ? "bg-[#e2e8f0] text-[var(--brand-neutral)] border-2 border-green-400 cursor-default"
                        ...
                        {bookingStatus && (
                          <Link 
                            ...
                            className="text-center text-sm font-bold text-[var(--brand-blue)] hover:text-[var(--brand-dark)] transition-colors uppercase tracking-widest underline decoration-2 underline-offset-4"
                          >
                            Ver mis reservas
                          </Link>
                        )}
              ...
              {externalOptions.length > 0 && (
                <div className="pt-10">
                  <h2 className="text-2xl font-bold text-[var(--brand-dark)] mb-8 flex items-center gap-3">
